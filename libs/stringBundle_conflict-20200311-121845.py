...
class StringBundle:
    ...
    def getBundle(cls, localeStr=None):
        if localeStr is None:
            try:
                print(locale.getlocale(), os.getenv('LANG'))
        ...
        localeStr = 'en'
        print(localeStr)
        strb = StringBundle(cls.__create_key, localeStr)
        getStrb = lambda strId: strb.getString(strId)
        print(getStrb('crtBox'))
    ...
    def __createLookupFallbackList(self, localeStr):
        ...
        print('Result paths', resultPaths)
        return resultPaths
